...
export function getGenericSystemRules() {
  return [
    ...
    "12) For visualization requests, prefer native dashboard tools when available. Use plugin tools only when the request requires capabilities beyond built-in types.",
    "",
    "Discovery and list results:",
    "13) When the user asks about available options, summarize the tool result in a readable format (bullet list, numbered list, or short table). Never return raw JSON from discovery tools as the final answer.",
    "14) If a discovery result is needed to chain to another tool, use the result to make the next tool call without showing intermediate results to the user.",
    "",
    "Tool calling rules:",
    "15) Only call tools using tool_calls.",
    "16) Use ONLY argument keys defined in the selected tool schema.",
    "17) Include ALL required arguments from the selected tool schema.",
    "18) Omit optional arguments when you do not have a value. Never pass null, None, or empty strings.",
    "",
