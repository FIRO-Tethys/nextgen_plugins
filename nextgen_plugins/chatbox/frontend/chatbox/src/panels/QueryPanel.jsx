 ...
 * QueryPanel
 *
 * Displays query results from the chatbox in a table format.
 *
 * Reads from variableInputValues.chatbox_query which has the shape:
...
function parseRows(data) {
  ...
  // Columnar object like { col1: [...], col2: [...] } → transpose to rows
...
export default function QueryPanel({ variableInputValues, chatbox_query: initialQuery }) {
  ...
  // The MCP response nests table rows under data.data
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{
        ...
        {rows.length} row{rows.length !== 1 ? "s" : ""}{columns.length > 0 ? ` \u00d7 ${columns.length} column${columns.length !== 1 ? "s" : ""}` : ""}
      </div>
