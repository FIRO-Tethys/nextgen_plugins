...
export function buildNrdsSystemMessage() {
  ...
  const nrdsRules = [
    ...
    "20) query_output_file and create_plotly_chart_from_parquet_output_file require ONE file URL, not a directory path.",
    ...
    "25) If the user asks for a chart using model/date/forecast/cycle/vpu, prefer create_plotly_chart_from_output_selector.",
    "26) Prefer selector tools over chaining resolve_output_file + another tool.",
    "",
    ...
    "- Prefer chart tools directly over query-then-text workflows.",
    "",
    "Output-file schema:",
    DATA_SCHEMA,
    "",
...
export function buildNrdsBeforeFirstMessage(text) {
  ...
  return {
    ...
    content: [
      ...
      'For chart requests, call create_plotly_chart_from_parquet_output_file with {"s3_url":"<file-url>","query":"<SQL>"}.',
      "SQL must query FROM output.",
    ].join("\n"),
  };
}
...
export const NRDS_AUTO_FIX_MSG = [
  ...
  "For charts use create_plotly_chart_from_output_selector or create_plotly_chart_from_parquet_output_file.",
